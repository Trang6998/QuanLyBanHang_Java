...
export class HTTPSingleton {
    ...
    setAccessToken(): void {
        if (store.state.user && store.state.user.AccessToken && store.state.user.Token.Token) {
            this.HTTP.defaults.headers.common['access_token'] = store.state.user.Token.Token;
        }
    }
}
